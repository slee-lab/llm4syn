...
# output=show_one_test(model0, dataset, idx, tokenizer, set_length={'type': out_type, 'value': out_size}, 
...
filename = f'./save/tgt2ceq_m0_{num_sample}_{tag}.csv'
save_dict_as_csv(chem_mean_dict, filename)
print(f"Dictionary saved as {filename}")
# save chem_dict as pkl
with open(f'./save/tgt2ceq_m0_{num_sample}_{tag}.pkl', 'wb') as f:
    pkl.dump(chem_dict, f)

from utils.periodic_trends import plotter
p = plotter(filename, output_filename=f'./save/tgt2ceq_m0_{num_sample}_{tag}.html', under_value=0, over_value=1)
